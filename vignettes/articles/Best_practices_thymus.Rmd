...
> This vignette predates the `correctEmbeddings` default added to
> `RunCanek()` in 0.3.0, so the one `RunCanek()` call below has been pinned
> to `correctEmbeddings = FALSE` to keep it runnable, matching what it
> originally did by default. Several of the careful manual steps this
> vignette walks through — double-checking the assay at each step, tuning
> the variable-feature intersection, confirming PC counts after correction
> — are less critical with 0.3.0's defaults (embedding reuse from an
> existing `"pca"` reduction, automatic `pcaDim` inference), though the
> underlying advice about batch labels and variable features is still
> generally sound.
...
corrected <- Canek::RunCanek(x = uncorrected, batches = "tech", features = integration_features, correctEmbeddings = FALSE)
```
