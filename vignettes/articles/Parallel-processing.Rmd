...
Parallelizing the two searches doesn't change what they compute, only when —
same seed, same input, same correction. `unname()` strips each reduction's
...
`"canekparallel_"` for the second run), which would otherwise trip
`all.equal()`'s default check even though every number matches.
